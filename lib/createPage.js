export default function createPage(options, ...children) {
    ...
    ogTags += options.description
        ...
        : "";
    let twitterTags = `<meta name="twitter:card" content="summary">\n<meta name="twitter:title" content="${options.title}">`;
...
<head>
  ...
  ${options.viewTransition
        ...
        : ""}
  <title>${options.title + (options.suffix ? ` | ${options.suffix}` : "")}</title>
  ${options.cssFiles
        ? options.cssFiles.map((file) => `<link rel="stylesheet" type="text/css" href="${file}" />`).join("\n")
        : ""}
  ${options.gtagId
        ? `<!-- Google tag (gtag.js) -->
